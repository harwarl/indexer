...
impl RawLog {
    ...
    async fn tx_hash(&self) -> Option<&str> { self.tx_hash.as_deref() }
    async fn log_index(&self) -> Option<i64> { self.log_index }
    async fn tx_index(&self) -> Option<i64> { self.tx_index }
    async fn address(&self) -> &str { &self.address }
    async fn topics(&self) -> &str { &self.topics }
    async fn data(&self) -> &str { &self.data }
}
